...
def getData():
    ...
    # handcrafted :  and m.itemid in [51006,50971,50882,50885,51300]
    #and m.itemid in [51006,51237,50983,50912,50868,50813] -- for reproducing paper
    lab_query= """MATCH (x:Patients)-[xr:ADMITTED]-(n:Admissions)-[r:HAS_LAB_EVENTS]->(m:D_Lab_Items) where n.hadm_id in"""+str(hadm_arr)+""" and x.subject_id in """+str(pat_arr)+""" and m.itemid in [51006,51237,50983,50912,50868,50813] and duration.inSeconds(datetime(n.admittime), datetime(r.charttime)).hours<= 24*10 RETURN  n.hospital_expire_flag as label,  r.value as value, r.valueUOM as units, n.marital_status as marital, n.ethnicity as ethnicity, n.religion as religion, m.fluid as fluid,  m.category as category, m.label as lab_name, n.hadm_id as adm_id,n.hadm_id as hadm_id, x.gender as gender,round(duration.inSeconds(datetime(n.admittime), datetime(r.charttime)).seconds*1.0/(60*60),2) as diff, duration.inSeconds(datetime(x.dob), datetime(n.admittime)).hours/8760 as age, m.itemid as lab_id, n.admission_type as adm_typ""" 
    ...
    # [618,51,114,198,646,677] 
    #and m.itemid in [44706,51,646] -- for reproducing paper
    vitals_query = """MATCH (n:Admissions)-[r:HAS_CHART_EVNT]->(m:D_Items) where n.hadm_id in """+str(hadm_arr)+""" and duration.inSeconds(datetime(n.admittime), datetime(r.charttime)).hours<= 24*10 and r.value<>'' RETURN n.hadm_id as hadm_id, m.label as name, m.itemid as itemid,r.value as value,r.valueUOM as UOM, r.charttime as time"""
    ...
    output_query = """MATCH (n:Admissions)-[r:HAS_OUT_EVNT]->(m:D_Items) where n.hadm_id in """+str(hadm_arr)+""" and duration.inSeconds(datetime(n.admittime), datetime(r.charttime)).hours<= 24*10 and r.value<>'' RETURN n.hadm_id as hadm_id, m.label as name, m.itemid as itemid,r.value as value,r.valueUOM as UOM, r.charttime as time"""
...
def getPreprocessData(type,edge_merge,grp_aggr):
    ...
    if type=='ML':
        ...
        return df_admission,df_diagnosis,df_drugs,df_labs,df_vitals,df_diagnosis_features,df_demo,df_output, weights
