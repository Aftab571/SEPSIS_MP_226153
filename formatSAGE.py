...
def create_train_val_test_mask(df):
    ...
    Y = df['label']
    mask =[]
    X_train_complete, X_test, y_train_complete, y_test = train_test_split(X,df['label'].values.tolist(), test_size=0.3,stratify=Y)
...
def main():
    ...
    df_admission,df_diagnosis,df_drugs,df_labs,df_vitals,df_diagnosis_features,df_demo,df_output,lst_weights = getPreprocessData('Graph',edge_merge=False,grp_aggr='mean')
    ...
    for aggr in ['sum']:
        ...
        for i in mask_list:
            ...
            data['Admission'].x = torch.tensor(df_admission[['gender','age', 'adm_typ']].values, dtype = torch.float).to(device)  #'ethnicity','marital','religion','gender','age'
